<template>
	<canvas id="myCanvas" type="2d" :style="{width: (width * ratio) + 'px', height: (height * ratio) + 'px'}"></canvas>
</template>
...
<script>
	...
	async function initPage(_this){
		const stage = await new Stage('#myCanvas', {width: 375, height: 360}, _this)
		...
		const pointerCircleDegree = (this.score / 100 * DEGREE_RANGE) + START_DEGREE // 根据百分制得出得分比算出指示圆位置角度
		...
		// 得分
		const textScoreFontSize = 56 * this.ratio
		...
		const textTips = new Text({
			font: `normal bold ${16 * this.ratio}px PingFang-SC`,
			text: '综合得分',
			color: 'white'
		})
