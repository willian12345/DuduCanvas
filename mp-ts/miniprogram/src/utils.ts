 ...
 *
 */
export function getPosAfterRotation(rotation: number, x: number, y: number) {
 ...
 */
export function getMaxValue(arr: {x:number, y: number}[]){
	let minX = arr[0].x
	let maxX = arr[0].x
	let minY = arr[0].y
	let maxY = arr[0].y
 ...
 * @param {*} value 
 */
export function getTransformedFourValue(value: number|string):number|number[]{
	// 如果是单个纯数字
	if(typeof value === 'number' && !isNaN(value)){
		return [value, value, value, value]
	}else if(typeof value === 'string'){
		let valueArr: string[]|number[] = value.split(' ');
